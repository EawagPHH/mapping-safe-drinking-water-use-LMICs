...
Here we model the four constituent subcomponents of safely managed drinking water:
(i) absence of faecal contamination (no detectable E. coli in a 100 ml sample of a household’s primary drinking water source),
(ii) availability when needed (no insufficiency of water quantity experienced within the last month), 
(iii) accessibility on premises (a water source located in the dwelling, yard or plot),  and 
(iv) use of an improved drinking water source (a drinking water source which provides some degree of protection from faecal contamination) 

```{r setup, include=FALSE}

# Set working directory
knitr::opts_chunk$set(echo = TRUE, message = FALSE, root.dir = 
"~/GitHub/mapping-safe-drinking-water-use-LMICs")
...
source("~/GitHub/mapping-safe-drinking-water-use-LMICs/Training/feature_names/39_features.R")
...
#Modelling freedom of faecal contamination using E.coli as an indicator
```{r}
...
Outcome: access to E.coli free water
```{r}
...
#write.csv(df.MICS_envir_human_e.coli_39, "/Users/Esther/Documents/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/Regre_Matrix_Ecoli.csv", fileEncoding = "UTF-8", row.names = F)
...
write.csv(rf_grid_perf_ecoli@summary_table,"~/GitHub/mapping-safe-drinking-water-use-LMICs/Training/rf_grid_ecoli_reg_39.csv")
```
...
write.csv(mean_values_ecoli, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/Shap_values_ecoli.csv", fileEncoding = "UTF-8", row.names = T)
```

Plotting summarized average SHAP values by Indicator Group for E.coli estimates
```{r}
...
##Plot results from best RF Model for E.coli contamination
Plot Predicted versus Observed E.coli contamination with results from leave-one-country-out holdout predictions.
```{r}
...
#write.csv(FullPrediction_ecoli, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/prediction-results/Fullprediction_Ecoli_newest.csv", fileEncoding = "UTF-8", row.names = F)
```
...
Ecoli_originalprediction <- read.csv( "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/prediction-results/Fullprediction_ecoli.csv", fileEncoding = "UTF-8")
...
```
## estimates at UNsubregion levels
```{r}
...
left_join(Ecoli_originalprediction, by = c("NAME_0"="NAME_0"))%>%
  dplyr::select(UNRegionName, UNSubRegionName,income_group, NAME_0, numberOfPeopleEcoliIncountry, percentageNoEcoli) %>% unique()
...
Here we create a dataframe with indicators for drinking water availability.
```{r}
...
##Grid-search: Tuning hyperparameters of random forest model with 39 features
Outcome: access to drinking water availability
```{r}
...
#write.csv(rf.grid.perf@summary_table,"~/GitHub/mapping-safe-drinking-water-use-LMICs/Training/results_grid_search/rf_grid_Avail_reg_39_newest.csv")
...
Plot Predicted versus Observed Availability with results from leave-one-country-out holdout predictions.
```{r}
...
#write.csv(mean_values_Avail, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/prediction_results/Shap_values_Avail_newest.csv", fileEncoding = "UTF-8", row.names = T)
...
Plotting summarized average SHAP values by Indicator Group for Availability
```{r}
...
Here we create a dataframe with indicators for drinking water accessibility.
```{r}
...
write.csv(combined_Access_envir_regMatrix_39 , "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/Regre_Matrix_Access.csv", fileEncoding = "UTF-8", row.names = F)
...
#write.csv(rf_grid_perf_Access@summary_table,"~/GitHub/mapping-safe-drinking-water-use-LMICs/Training/grid_search_results/rf_grid_Access_reg_39_newest.csv")
...
Plot Predicted versus Observed Accessibility with results from leave-one-country-out holdout predictions.
```{r}
...
#write.csv(mean_values_Access, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/prediction_results/Shap_values_Access_newest.csv", fileEncoding = "UTF-8", row.names = T)
```
...
#write.csv(FullPrediction_Access, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/prediction_results/FullPrediction_Access_newest.csv", fileEncoding = "UTF-8", row.names = F)
```
##Descriptive analysis of accessibility results
```{r}
#source("~/GitHub/mapping-safe-drinking-water-use-LMICs/Data_cleaning/MICS_Preprocessing_Dataframes/Compiling_MICS_test_and_training_sets/functionsForJoiningAndStructuringDataframes.R")

Access_prediction <- read.csv( "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/prediction_results/Fullprediction_Access.csv", fileEncoding = "UTF-8")
...
#Random Forest Model - Outcome: water source type
Here we create a dataframe with indicators for drinking water source type (proportion of people using an improved drinking water source).
```{r}
...
#write.csv(rf_grid_perf_improved@summary_table,"~/GitHub/mapping-safe-drinking-water-use-LMICs/Training/grid_search_results_rf_grid_improved_reg_39_newest.csv")
...
```
Plot predicted versus Observed populations using improved drinking water sources with results from leave-one-country-out holdout predictions.
```{r}
...
#write.csv(mean_values_Improved, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/predition_results/Shap_values_Improved_newest.csv", fileEncoding = "UTF-8", row.names = T)
```

Plotting summarized average SHAP values by Indicator Group for Water Source Type
```{r}
...
##Improved drinking water source prediction at global administrative (GADM) area level 1
```{r}
#level 1 prediction
...
#write.csv(FullPrediction_Improved, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/prediction_results/FullPrediction_Improved_newest.csv", fileEncoding = "UTF-8", row.names = F)
```
...
Improved_prediction <- read.csv( "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/predition_results/Fullprediction_improved.csv", fileEncoding = "UTF-8")
