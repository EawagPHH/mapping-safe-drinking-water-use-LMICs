...
ggplot(aes(feature, contribution, color = normalized_value, text = row)) +
        ...
        coord_flip() +
        labs(y = "SHAP contribution to E. coli absence in drinking water source prediction", x = "", col = "Normalized\nfeature value") +
...
TrainAndPredicted_ecoli_withGlobalRegions <- TrainAndPredicted_ecoli %>% left_join(GlobalRegions, by = c("ISO3.Alpha.code"="ISO.alpha3.Code"))

TrainAndPredicted_ecoli_withGlobalRegions$UNRegionName[TrainAndPredicted_ecoli_withGlobalRegions$Country == "Kosovo (under UNSC res. 1244)"] <- "Europe"

# Plot predicted vs observed values

colours <- c("#E64B35FF","#91D1C2FF","#3C5488FF","#4DBBD5FF", "#EFC00F")
                                    
plotPredObs_ecoli <- ggplot(TrainAndPredicted_ecoli_withGlobalRegions, 
                      aes(x = predict, 
                          y = train, size = nr_HH_in_region, colour= UNRegionName)) +
  geom_point(shape=1) +
...
UNSubregion_original_ProportionNoEcoli 
...
#write.csv(combined_Avail_envir_regMatrix_39, here("5_Predictions/prediction_results/Regre_Matrix_Avail.csv"), fileEncoding = "UTF-8", row.names = F)
...
TrainAndPredicted_Avail_withGlobalRegions <- TrainAndPredicted_Avail %>% left_join(GlobalRegions, by = c("ISO3.Alpha.code"="ISO.alpha3.Code"))

TrainAndPredicted_Avail_withGlobalRegions$UNRegionName[TrainAndPredicted_Avail_withGlobalRegions$Country == "Kosovo (under UNSC res. 1244)"] <- "Europe"

# Plot predicted vs observed values
                                    
plotPredObs_Avail <- ggplot(TrainAndPredicted_Avail_withGlobalRegions, 
                      aes(x = predict, 
                          y = train, size = nr_HH_in_region, colour= UNRegionName)) +
  geom_point(shape=1) +
...
shap_summary_plot_data <- shap_summary_plot_avail[['data']]
shap_summary_plot_data$feature <- as.character(shap_summary_plot_data$feature)
shap_summary_plot_data <-rename39Features(shap_summary_plot_data)

shap_summary_plot_data <- shap_summary_plot_data %>%
  left_join(mean_values_Avail, by = c("feature"="feature"))
...
ggplot(aes(feature, contribution, color = normalized_value, text = row)) +
        ...
        scale_fill_gradientn(colors = viridis_pal()(9), breaks=c(0,1),labels=c("low","high"), limits=c(0, 1)) +
        coord_flip() +
        labs(y = "SHAP contribution to availability when needed prediction", x = "", col = "Normalized\nfeature value") +
        theme_bw(base_size = 9) 
...
TrainAndPredicted_Access_withGlobalRegions <- TrainAndPredicted_Access %>% left_join(GlobalRegions, by = c("ISO3.Alpha.code"="ISO.alpha3.Code"))

TrainAndPredicted_Access_withGlobalRegions$UNRegionName[TrainAndPredicted_Access_withGlobalRegions$Country == "Kosovo (under UNSC res. 1244)"] <- "Europe"


# Plot predicted vs observed values

plotPredObs_Access <- ggplot(TrainAndPredicted_Access_withGlobalRegions, 
                      aes(x = predict, 
                          y = train, size = nr_HH_in_region, colour= UNRegionName)) +
  geom_point(shape=1) +
  labs(x = "Predicted proportion of population with drinking water on premises",
       y = "Observed proportion of population with drinking water on premises") +
  coord_cartesian(xlim = c(0,1), ylim = c(0,1)) +
stat_smooth(se = T,
              colour = "black",
              size = 0.5,
...
shap_summary_plot_data <- shap_summary_plot_Access[['data']]
shap_summary_plot_data$feature <- as.character(shap_summary_plot_data$feature)
shap_summary_plot_data <-rename39Features(shap_summary_plot_data)

shap_summary_plot_data <- shap_summary_plot_data %>%
  left_join(mean_values_Access, by = c("feature"="feature"))
...
ggplot(aes(feature, contribution, color = normalized_value, text = row)) +
        ...
        scale_fill_gradientn(colors = viridis_pal()(9), breaks=c(0,1),labels=c("low","high"), limits=c(0, 1)) +
        coord_flip() +
        labs(y = "SHAP contribution to accessibility on premises prediction", x = "", col = "Normalized\nfeature value") +
        theme_bw(base_size = 9) 
...
# select only variables needed for random forest models  
combined_Improved_envir_regMatrix_39 <- combined_Improved_envir_human_39 %>%
  select(all_of(bandNames_39),all_of(varToModel_improved),HouseholdsInRegion.Freq.x, country_fold) 

regMatrixH2O_improved_reg_39 <- as.h2o(combined_Improved_envir_regMatrix_39, destination_frame = "regMatrixH2O_improved_reg_39")

#write.csv(combined_Improved_envir_regMatrix_39, here("5_Predictions/prediction_results/Regre_Matrix_Improved.csv"), fileEncoding = "UTF-8", row.names = F)
...
TrainAndPredicted_Improved_withGlobalRegions <- TrainAndPredicted_Improved %>% left_join(GlobalRegions, by = c("ISO3.Alpha.code"="ISO.alpha3.Code"))

TrainAndPredicted_Improved_withGlobalRegions$UNRegionName[TrainAndPredicted_Improved_withGlobalRegions$Country == "Kosovo (under UNSC res. 1244)"] <- "Europe"


# Plot predicted vs observed values
                                    
plotPredObs_Improved <- ggplot(TrainAndPredicted_Improved_withGlobalRegions, 
                      aes(x = predict, 
                          y = train, size = nr_HH_in_region, colour= UNRegionName)) +
  geom_point(shape=1) +
  labs(x = "Predicted proportion of population using an improved source",
       y = "Observed proportion of population using an improved source") +
  coord_cartesian(xlim = c(0,1), ylim = c(0,1)) +
stat_smooth(se = T,
              colour = "black",
              size = 0.5,
