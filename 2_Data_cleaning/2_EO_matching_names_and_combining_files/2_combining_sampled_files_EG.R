...
matched_names <- fread("./2_Data_cleaning/2_EO_matching_names_and_combining_files/matched_names_EG_2021.02.04.csv") %>% 
  select(-name_orig) %>%
...
unmatched_names <- fread("./2_Data_cleaning/2_EO_matching_names_and_combining_files/unmatched_names_afterF.csv") %>% 
  select(-name_orig) %>%
  filter(accepted == 1) %>% mutate(matched_name1 = coalesce(NAME_1_matched, NAME_1_matched_fuzz)) %>% 
  mutate(matched_name2 = coalesce(NAME_2_matched, NAME_2_matched_fuzz)) %>% 
  mutate(matched_name3 = coalesce(NAME_3_matched, NAME_3_matched_fuzz))
...
NAME_1_sampled <- fread("./2_Data_cleaning/2_EO_matching_names_and_combining_files/NAME_1_sampled.csv")
NAME_2_sampled <- fread("./2_Data_cleaning/2_EO_matching_names_and_combining_files/NAME_2_sampled.csv")
NAME_3_sampled <- fread("./2_Data_cleaning/2_EO_matching_names_and_combining_files/NAME_3_sampled.csv")
...
#fwrite(./2_Data_cleaning/2_EO_matching_names_and_combining_files/all_name_sampled, "MICS_env_sampled_.csv")
