...
hh_Benin <- extractStandardSurveyVariables(hh_Benin)
...
hh_Benin_HH7extract <- extractVariableLabelsfromHH7(hh_Benin)
...
hh_Benin_WS1extract <- extractVariableLabelsfromWS1(hh_Benin)
...
hh_Benin_HH7extract$id <- as.factor(hh_Benin_HH7extract$id)
...
hh_Benin <- replaceHH7LabelsWithRegionNames(hh_Benin,hh_Benin_HH7extract)
...
hh_Benin <-VariablesWhichWereFactorsAsCharacterToAvoidGenerationOfNA(hh_Benin)
...
df.MICS.SMDW_test <- rbind(hh_Benin,hh_Congo, hh_DominicanRepublic, hh_Fiji, 
...
write.csv(df.MICS.SMDW_testLabeled, "df_SMDW_test_final_revised.csv", fileEncoding = "UTF-8", row.names = F)
