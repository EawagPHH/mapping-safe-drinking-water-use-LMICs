...
```{r}
source("~/Documents/GitHub/mapping-safe-drinking-water-use-LMICs/2_Data_cleaning/3_MICS_Preprocessing_Dataframes/1_Compiling_MICS_test_and_training_sets/functionsForJoiningAndStructuringDataframes.R")

#(1)
df.MICS_test <- readHouseHoldSurveyDataForTestSet()

#congo was removed from test set as no information on water availability was present
df.MICS_test <- df.MICS_test %>% 
  subset(country != "Congo")
#(2)
...
```
Next we match our SMDW predictions at GADM area level 1 with our calculated SMDW coverage rates at a) HH7_region level and b) country level. We do this for predictions generated from our best performing random forest model built on the original training set of 39 geospatial covariates as well as for the average prediction of 100 random forest models build on geographically stratified training sets.
...
#Load and join population counts for 2020 to prediction data frames
...
#Join prediction data frames with matched HH7_region names
...
colours <- c("#E64B35FF","#91D1C2FF","#3C5488FF","#EFC00F")
