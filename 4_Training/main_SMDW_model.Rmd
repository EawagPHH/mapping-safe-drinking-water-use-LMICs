...
# Modeling Safely Managed Drinking Water in Low- and Middle- Income Countries

Here we model access to safely managed drinking water (SMDW) using random forest models with geospatial and Multiple Indicator Cluster Survey (MICS) data.
...
## 1.2 Combining SMDW indicator from MICS data with Earth Observation derived data

```{r}
EO_features <- renameDuplicateHH7RegionNamesFromDifferentCountries(EO_features)

source(here("./4_Training/feature_names/24_features.r"))
source(here("./4_Training/feature_names/39_features.R"))
source(here("./4_Training/feature_names/51_features.R"))
source(here("./4_Training/feature_names/73_features.R"))
source(here("./4_Training/feature_names/91_features.R"))
source(here("./4_Training/feature_names/117_features.R"))
...
## 2.1. Random forest model with all 117 pre-selected features
...
## 2.2 Random forest model with 73 pre-selected features
...
combined_SMDW_envir_regMatrix_73 <- combined_SMDW_envir_human_73%>%
  dplyr::select(all_of(bandNames_73),all_of(varToModel_SMDW),HouseholdsInRegion.Freq.x, country_fold)%>% drop_na(country_fold)
...
## 2.3 Random forest model with 51 pre-selected features
...
## 2.4 Random forest model with 39 pre-selected features
...
## 2.5 Random forest model with 24 pre-selected features
...
# 3. Grid-search: Tuning hyperparameters of random forest model with 39 features

We conducted a grid-search to iteratively explore the results of a suite of random forest models trained on the selected covariates. A list of hyperparameters was selected, varying in (1) the number of trees, (2) the number of variables sampled at each split and (3) the minimum observations per leaf and (4) sampling rate.
...
#write.csv(rf.grid.perf@summary_table,"./4_Training/results_grid_search/rf_grid_SMDW_reg_39.csv")

h2o.shutdown()
```

# 4. Best Random Forest model for SMDW using 39 features (environmental and human indicators)
...
#write.csv(combined_SMDW_envir_regMatrix_39, "./5_Prediction/prediction_results/Regre_Matrix_SMDW.csv", fileEncoding = "UTF-8", row.names = T)
...
## 4.1 Plot results from best RF Model for SMDW

Plot Predicted versus Observed SMDW usage with results from leave-one-country-out holdout predictions.

```{r}
#create data frame with validation holdout predictions, observed SMDW values
...
colours <- c("#E64B35FF","#91D1C2FF","#3C5488FF","#4DBBD5FF", "#EFC00F")


plotPredObs_SMDW <- ggplot(TrainAndPredicted_SMDW_withGlobalRegions, 
                      aes(x = predict, 
                          y = SMDWcoverageAtRegionalLevel, size = NumberOfHouseholdsInRegion, colour=UNRegionName )) +
  geom_point(shape=1) +
...
## 4.2 Compute SHAP values and plot SHAP contribution summary
...
#write.csv(mean_SHAPvalues_SMDW, "./5_Prediction/prediction_results/Shap_values_SMDW_39_newest.csv", fileEncoding = "UTF-8", row.names = T)
...
## 4.3 Plotting summarized average SHAP values by Indicator Group
...
## 4.4 Random-fold cross validation results
...
# 5. Evaluating model performance on different feature groups

## 5.1 Modelling with only human versus only environmental features
...
## 5.2 Modelling with human, environmental and 2 additional governance features

```{r}
library(readxl)
WGI_indicator <- read_excel("~/switchdrive/PHD-2020-2024/SMDWs submission/Data_for_reviewers/WGIdata_WB.xlsx")

combined_SMDW_envir_human_39_WGI <- combined_SMDW_envir_human_39  %>% 
  left_join(WGI_indicator, by=(c("NAME_0"="Country")))

combined_SMDW_envir_human_39_WGI$`Government_Effectiveness` <- combined_SMDW_envir_human_39_WGI$`Government Effectiveness`+2

combined_SMDW_envir_human_39_WGI$`Regulatory_Quality` <- combined_SMDW_envir_human_39_WGI$`Regulatory Quality`+2

# select only variables needed for random forest models  
combined_SMDW_WGI_regMatrix_39 <- combined_SMDW_envir_human_39_WGI %>%
  select(all_of(bandNames_39),all_of(varToModel_SMDW),HouseholdsInRegion.Freq.x, 'Government_Effectiveness', 'Regulatory_Quality',country_fold)

#save regression Matrix as H2O data frames  
regMatrixH2O_SMDW_WGI <- as.h2o(combined_SMDW_WGI_regMatrix_39, destination_frame = "regMatrixH2O_SMDW_WGI")

# random forest regression- with leave-one-country-out cross-validation
rf_model_SMDW_WGI <- h2o.randomForest(
  y = varToModel_SMDW,
  training_frame = regMatrixH2O_SMDW_WGI,
  ntrees = 200,
  mtries = 10,
  sample_rate = 0.6,
   min_rows = 3,
...
h2o.r2(rf_model_SMDW_WGI, xval = FALSE) # metrics computed using full training set 
h2o.mae(rf_model_SMDW_WGI, xval = FALSE)

h2o.r2(rf_model_SMDW_WGI, xval = TRUE) # metrics computed based on leave-one-country-out cross-validation
h2o.mae(rf_model_SMDW_WGI, xval = TRUE)

```

### 5.2.1 Calculating SHAP contributions for governance models on training set

```{r}
shap_values_SMDW_WGI<- predict_contributions.H2OModel(
  rf_model_SMDW_WGI,
  regMatrixH2O_SMDW_WGI)

mean_SHAPvalues_SMDW_WGI <- as.data.frame(colMeans(abs(as.data.frame(shap_values_SMDW_WGI) %>% select(-BiasTerm)))); names(mean_SHAPvalues_SMDW_WGI) <- 'Mean'
mean_SHAPvalues_SMDW_WGI %>% arrange(desc(Mean))

mean_SHAPvalues_SMDW_WGI <- cbind(feature = rownames(mean_SHAPvalues_SMDW_WGI), mean_SHAPvalues_SMDW_WGI)

mean_SHAPvalues_SMDW_WGI  <- rename41Features(mean_SHAPvalues_SMDW_WGI)

shap_summary_plot_WGI <- h2o.shap_summary_plot(rf_model_SMDW_WGI, regMatrixH2O_SMDW_WGI)
print(shap_summary_plot_WGI)

shap_summary_plot_data_WGI <- shap_summary_plot_WGI[['data']]
shap_summary_plot_data_WGI$feature <- as.character(shap_summary_plot_data_WGI$feature)
shap_summary_plot_data_WGI <-rename41Features(shap_summary_plot_data_WGI)

shap_summary_plot_data_WGI <- shap_summary_plot_data_WGI %>%
  left_join(mean_SHAPvalues_SMDW_WGI, by = c("feature"="feature"))


shap_summary_plot_data_WGI %>%
  mutate(feature = fct_reorder(feature, Mean)) %>%
ggplot(aes(feature, contribution, color = normalized_value, text = row)) +
        geom_hline(yintercept = 0, linetype = "dashed") +
        geom_point(position = h2o:::position_jitter_density(), alpha = 0.5, size = 1.2) +
...
## 5.3 Plotting relationship between Governance indicators and selected EO indicators and SMDW (for interest, not included in manuscript)

```{r}
TrainAndPredicted_SMDW_WGI <- createDataFrameWithValidationHoldOutPredictions(rf_model_SMDW_WGI, combined_SMDW_envir_human_39_WGI) 

TrainAndPredicted_SMDW_WGI <- joinWithCountryIncomeGroups(TrainAndPredicted_SMDW_WGI,UN_Population_Devision_Names, country_Income_Groups)

TrainAndPredicted_SMDW_WGI$GovernmentEffectiveness <- combined_SMDW_envir_human_39_WGI$`Government Effectiveness`-2

TrainAndPredicted_SMDW_WGI$RegulatorQuality <- combined_SMDW_envir_human_39_WGI$`Regulatory Quality`-2

TrainAndPredicted_SMDW_WGI$UrbanBuiltUp <- combined_SMDW_envir_human_39_WGI$ConsensusLandCoverClass_Urban_Builtup

TrainAndPredicted_SMDW_WGI$PopulationDensity <- combined_SMDW_envir_human_39_WGI$GPWv4_Population_Density


TrainAndPredicted_SMDW_WGI$BurntAreas <- combined_SMDW_envir_human_39_WGI$EsaCci_BurntAreasProbability

TrainAndPredicted_SMDW_WGI$Temperature <- combined_SMDW_envir_human_39_WGI$CHELSA_BIO_Annual_Mean_Temperature

TrainAndPredicted_SMDW_WGI$Precipitation <- combined_SMDW_envir_human_39_WGI$CHELSA_BIO_Annual_Precipitation


colours <- c("#E64B35FF", "#91D1C2FF","#3C5488FF","#4DBBD5FF", "#00A087FF")
#show_col(pal_npg("nrc")(10))

ggplot(TrainAndPredicted_SMDW_WGI, 
                      aes(x = GovernmentEffectiveness, 
  ...
  labs(x = "Government Effectiveness",
   ...
   coord_cartesian(xlim = c(-2,1), ylim = c(0,1)) +
  stat_smooth(se = T,
              colour = "black",
              size = 0.5,
              method = "lm") +
...
# 6. Predicting SMDW use at global administrative (GADM) area level 1
...
#write.csv(FullPrediction_SMDW, "~/GitHub/mapping-safe-drinking-water-use-LMICs/Prediction/Fullprediction_SMDW_newest.csv", fileEncoding = "UTF-8", row.names = F)
...
## 7.1 Evaluating data distribution across income levels and UN regions
...
## 7.2 Boot-strapping samples from training set with UN Global Regions as stratification categories
...
rf_predictions$percentile75 <- apply(rf_predictions[,1:100],1, quantile, probs= 0.75)
 ...
 #write.csv(rf_predictions, "./5_Prediction/prediction_results/100Bootstrap_SMDWPredictions.csv", fileEncoding = "UTF-8", row.names = F) 
...
#write.csv(rf_predictions_IQR, "./5_Prediction/prediction_results/IQR_CoV_Mean_ofSMDWModels_newest.csv", fileEncoding = "UTF-8", row.names = F) 
 
```

# 8. Descriptive analysis of results
...
## 8.1 Calculating SMDW coverage and total numbers of users at global and UN region levels

```{r}
rf_predictions <- read.csv(here("./5_Predictions/prediction_results/100Bootstrap_SMDWPredictions.csv"))
...
#write.csv(FullPrediction_SMDW_UNGlobalRegions,"./5_Prediction/prediction_results/SMDWPredictions_proportionAndNumberOfPeople_newest.csv") 
...
## 8.2 Calculating SMDWS coverage and total numbers of users at national levels
...
#write.csv(nationalpopulationSMDWcoverage_,"./5_Prediction/prediction_results/PredictionResultsNational_final_incl_mean_percentiles_newest.csv") 
```

# 9. Comparing national predicted estimates with JMP Data

## 9.1 Plotting SMDW predictions at national level against JMP national estimates

```{r}
JMP_SMDWs <- read.csv("~/switchdrive/PHD-2020-2024/SMDWs submission/Data_for_reviewers/JMP_2020_SMDWs_extracted.csv", encoding = "UTF-8") 
...
# joining with UN population devision names and income group levels
nationalPredictions_JMP_SMDWs_omitNA <- nationalPredictions_JMP_SMDWs %>% 
  select(-Training,-Validation,-X.U.FEFF.Country,-nationalSMDWcoverage_Hhlevel) %>% 
  na.omit() %>%
  left_join(UN_Population_Devision_Names, by = c("country"="Country")) %>%
  left_join(country_Income_Groups, by = c("ISO3.Alpha.code"="Code"))

# manually assign income group to a countries which could not be matched with the ISO3 alpha code

nationalPredictions_JMP_SMDWs_omitNA$Income.group[nationalPredictions_JMP_SMDWs_omitNA$country == "Iran"] <- 'Lower middle income'
nationalPredictions_JMP_SMDWs_omitNA$Income.group[nationalPredictions_JMP_SMDWs_omitNA$country == "Laos"] <- 'Lower middle income'
nationalPredictions_JMP_SMDWs_omitNA$Income.group[nationalPredictions_JMP_SMDWs_omitNA$country == "Palestina"] <- 'Lower middle income'
nationalPredictions_JMP_SMDWs_omitNA$Income.group[nationalPredictions_JMP_SMDWs_omitNA$country == "Russia"] <- 'Upper middle income'
...
plotPredictionVsJMP_Income <- ggplot(nationalPredictions_JMP_SMDWs_omitNA, 
  ...
  geom_point(shape=1) +
  labs(x = "Predicted proportion of population using SMDWS",
       y = "JMP reported proportion of population using SMDWS", size= "national Population", col = "Income group") +
   coord_cartesian(xlim = c(0,1), ylim = c(0,1)) +
 stat_smooth(se = T,
              colour = "black",
              size = 0.5,
...
plotPredictionVsJMP_UNRegion <- ggplot(nationalPredictions_JMP_SMDWs_omitNA, 
  ...
  geom_point(shape=1) +
  labs(x = "Predicted proportion of population using SMDWS",
       y = "JMP reported proportion of population using SMDWS", size= "National population", col = "UN region" ) +
   coord_cartesian(xlim = c(0,1), ylim = c(0,1)) +
 stat_smooth(se = T,
             method = "lm",
             colour = "black",
  ...
  geom_abline(slope = 1,
              ...
              linetype = "dashed") +
   #geom_linerange(data = nationalPredictions_JMP_SMDWs_omitNA, aes(xmin = nationalSMDWcoverage_IQR_25, xmax = nationalSMDWcoverage_IQR_75, size=.3))+
  theme_minimal(base_size = 9) + scale_colour_manual(values = colours)


plotPredictionVsJMP_UNRegion
...
#write.csv(nationalPredictions_JMP_SMDWs,"~./5_Prediction/prediction_results/nationalPredictions_JMP_SMDWs_newest.csv") 
```

## 9.2 Analysis of differences in JMP estimates and random forest predictions
...
#for all SMDW observation from main model (single prediction) and JMP estimates
r2_JMP_singlePred <- 1 - (sum((nationalPredictions_JMP_SMDWs_omitNA$JMP.reported-nationalPredictions_JMP_SMDWs_omitNA$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_SMDWs_omitNA$JMP.reported-mean(nationalPredictions_JMP_SMDWs_omitNA$JMP.reported))^2))
r2_JMP_singlePred

MAE_singlePred <- sum(abs(nationalPredictions_JMP_SMDWs_omitNA$JMP.reported - nationalPredictions_JMP_SMDWs_omitNA$nationalSMDWcoverage))/length(nationalPredictions_JMP_SMDWs_omitNA$JMP.reported)
MAE_singlePred

#for all SMDW observation from main model (ensemble prediction) and JMP estimates
r2_JMP_singlePred <- 1 - (sum((nationalPredictions_JMP_SMDWs_omitNA$JMP.reported-nationalPredictions_JMP_SMDWs_omitNA$nationalSMDWcoverage_mean)^2)/sum((nationalPredictions_JMP_SMDWs_omitNA$JMP.reported-mean(nationalPredictions_JMP_SMDWs_omitNA$JMP.reported))^2))
r2_JMP_singlePred

MAE_singlePred <- sum(abs(nationalPredictions_JMP_SMDWs_omitNA$JMP.reported - nationalPredictions_JMP_SMDWs_omitNA$nationalSMDWcoverage_mean))/length(nationalPredictions_JMP_SMDWs_omitNA$JMP.reported)
MAE_singlePred

##for low income SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_lowIncome <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(Income.group == "Low income")

r2_JMP_singlePred_LI <- 1 - (sum((nationalPredictions_JMP_lowIncome$JMP.reported-nationalPredictions_JMP_lowIncome$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_lowIncome$JMP.reported-mean(nationalPredictions_JMP_lowIncome$JMP.reported))^2))
r2_JMP_singlePred_LI

MAE_singlePred_LI <- sum(abs(nationalPredictions_JMP_lowIncome$JMP.reported - nationalPredictions_JMP_lowIncome$nationalSMDWcoverage))/length(nationalPredictions_JMP_lowIncome$JMP.reported)
MAE_singlePred_LI

##for low income SMDW from main model (ensemble prediction) and JMP estimates

r2_JMP_singlePred_LI <- 1 - (sum((nationalPredictions_JMP_lowIncome$JMP.reported-nationalPredictions_JMP_lowIncome$nationalSMDWcoverage_mean)^2)/sum((nationalPredictions_JMP_lowIncome$JMP.reported-mean(nationalPredictions_JMP_lowIncome$JMP.reported))^2))
r2_JMP_singlePred_LI

MAE_singlePred_LI <- sum(abs(nationalPredictions_JMP_lowIncome$JMP.reported - nationalPredictions_JMP_lowIncome$nationalSMDWcoverage_mean))/length(nationalPredictions_JMP_lowIncome$JMP.reported)
MAE_singlePred_LI

##for lower middle income SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_lowMiddleIncome <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(Income.group == "Lower middle income")

r2_JMP_singlePred_LMI <- 1 - (sum((nationalPredictions_JMP_lowMiddleIncome$JMP.reported-nationalPredictions_JMP_lowMiddleIncome$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_lowMiddleIncome$JMP.reported-mean(nationalPredictions_JMP_lowMiddleIncome$JMP.reported))^2))
r2_JMP_singlePred_LMI

MAE_singlePred_LMI <- sum(abs(nationalPredictions_JMP_lowMiddleIncome$JMP.reported - nationalPredictions_JMP_lowMiddleIncome$nationalSMDWcoverage))/length(nationalPredictions_JMP_lowMiddleIncome$JMP.reported)
MAE_singlePred_LMI


##for upper middle income SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_UpMiddleIncome <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(Income.group == "Upper middle income")

r2_JMP_singlePred_UMI <- 1 - (sum((nationalPredictions_JMP_UpMiddleIncome$JMP.reported-nationalPredictions_JMP_UpMiddleIncome$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_UpMiddleIncome$JMP.reported-mean(nationalPredictions_JMP_UpMiddleIncome$JMP.reported))^2))
r2_JMP_singlePred_UMI

MAE_singlePred_UMI <- sum(abs(nationalPredictions_JMP_UpMiddleIncome$JMP.reported - nationalPredictions_JMP_UpMiddleIncome$nationalSMDWcoverage))/length(nationalPredictions_JMP_UpMiddleIncome$JMP.reported)
MAE_singlePred_UMI

##for Africa SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_Africa <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(UNRegionName == "Africa")

r2_JMP_singlePred_Africa <- 1 - (sum((nationalPredictions_JMP_Africa$JMP.reported-nationalPredictions_JMP_Africa$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_Africa$JMP.reported-mean(nationalPredictions_JMP_Africa$JMP.reported))^2))
r2_JMP_singlePred_Africa

MAE_singlePred_Africa <- sum(abs(nationalPredictions_JMP_Africa$JMP.reported - nationalPredictions_JMP_Africa$nationalSMDWcoverage))/length(nationalPredictions_JMP_Africa$JMP.reported)
MAE_singlePred_Africa

##for Africa SMDW from main model (ensemble prediction) and JMP estimates

r2_JMP_ensemblePred_Africa <- 1 - (sum((nationalPredictions_JMP_Africa$JMP.reported-nationalPredictions_JMP_Africa$nationalSMDWcoverage_mean)^2)/sum((nationalPredictions_JMP_Africa$JMP.reported-mean(nationalPredictions_JMP_Africa$JMP.reported))^2))
r2_JMP_ensemblePred_Africa

MAE_ensemblePred_Africa <- sum(abs(nationalPredictions_JMP_Africa$JMP.reported - nationalPredictions_JMP_Africa$nationalSMDWcoverage_mean))/length(nationalPredictions_JMP_Africa$JMP.reported)
MAE_ensemblePred_Africa

##for Asia SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_Asia <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(UNRegionName == "Asia")

r2_JMP_singlePred_Asia <- 1 - (sum((nationalPredictions_JMP_Asia$JMP.reported-nationalPredictions_JMP_Asia$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_Asia$JMP.reported-mean(nationalPredictions_JMP_Asia$JMP.reported))^2))
r2_JMP_singlePred_Asia

MAE_singlePred_Asia <- sum(abs(nationalPredictions_JMP_Asia$JMP.reported - nationalPredictions_JMP_Asia$nationalSMDWcoverage))/length(nationalPredictions_JMP_Asia$JMP.reported)
MAE_singlePred_Asia

##for Asia SMDW from main model (ensemble prediction) and JMP estimates

r2_JMP_ensemblePred_Asia <- 1 - (sum((nationalPredictions_JMP_Asia$JMP.reported-nationalPredictions_JMP_Asia$nationalSMDWcoverage_mean)^2)/sum((nationalPredictions_JMP_Asia$JMP.reported-mean(nationalPredictions_JMP_Asia$JMP.reported))^2))
r2_JMP_ensemblePred_Asia

MAE_ensemblePred_Asia <- sum(abs(nationalPredictions_JMP_Asia$JMP.reported - nationalPredictions_JMP_Asia$nationalSMDWcoverage_mean))/length(nationalPredictions_JMP_Asia$JMP.reported)
MAE_ensemblePred_Asia

##for Americas SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_Americas <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(UNRegionName == "Americas")

r2_JMP_singlePred_Americas <- 1 - (sum((nationalPredictions_JMP_Americas$JMP.reported-nationalPredictions_JMP_Americas$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_Americas$JMP.reported-mean(nationalPredictions_JMP_Americas$JMP.reported))^2))
r2_JMP_singlePred_Americas

MAE_singlePred_Americas <- sum(abs(nationalPredictions_JMP_Americas$JMP.reported - nationalPredictions_JMP_Americas$nationalSMDWcoverage))/length(nationalPredictions_JMP_Americas$JMP.reported)
MAE_singlePred_Americas

##for Americas SMDW from main model (ensemble prediction) and JMP estimates

r2_JMP_ensemblePred_Americas <- 1 - (sum((nationalPredictions_JMP_Americas$JMP.reported-nationalPredictions_JMP_Americas$nationalSMDWcoverage_mean)^2)/sum((nationalPredictions_JMP_Americas$JMP.reported-mean(nationalPredictions_JMP_Americas$JMP.reported))^2))
r2_JMP_ensemblePred_Americas

MAE_ensemblePred_Americas <- sum(abs(nationalPredictions_JMP_Americas$JMP.reported - nationalPredictions_JMP_Americas$nationalSMDWcoverage_mean))/length(nationalPredictions_JMP_Americas$JMP.reported)
MAE_ensemblePred_Americas

##for Europe SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_Europe <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(UNRegionName == "Europe")

r2_JMP_singlePred_Europe <- 1 - (sum((nationalPredictions_JMP_Europe$JMP.reported-nationalPredictions_JMP_Europe$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_Europe$JMP.reported-mean(nationalPredictions_JMP_Europe$JMP.reported))^2))
r2_JMP_singlePred_Europe

MAE_singlePred_Europe <- sum(abs(nationalPredictions_JMP_Europe$JMP.reported - nationalPredictions_JMP_Europe$nationalSMDWcoverage))/length(nationalPredictions_JMP_Europe$JMP.reported)
MAE_singlePred_Europe

##for Europe SMDW from main model (ensemble prediction) and JMP estimates

r2_JMP_ensemblePred_Europe <- 1 - (sum((nationalPredictions_JMP_Europe$JMP.reported-nationalPredictions_JMP_Europe$nationalSMDWcoverage_mean)^2)/sum((nationalPredictions_JMP_Europe$JMP.reported-mean(nationalPredictions_JMP_Europe$JMP.reported))^2))
r2_JMP_ensemblePred_Europe

MAE_ensemblePred_Europe <- sum(abs(nationalPredictions_JMP_Europe$JMP.reported - nationalPredictions_JMP_Europe$nationalSMDWcoverage_mean))/length(nationalPredictions_JMP_Europe$JMP.reported)
MAE_ensemblePred_Europe


##for Oceania SMDW from main model (single prediction) and JMP estimates
nationalPredictions_JMP_Oceania <- nationalPredictions_JMP_SMDWs_omitNA %>%
  subset(UNRegionName == "Oceania")

r2_JMP_singlePred_Oceania <- 1 - (sum((nationalPredictions_JMP_Oceania$JMP.reported-nationalPredictions_JMP_Oceania$nationalSMDWcoverage)^2)/sum((nationalPredictions_JMP_Oceania$JMP.reported-mean(nationalPredictions_JMP_Oceania$JMP.reported))^2))
r2_JMP_singlePred_Oceania

MAE_singlePred_Oceania <- sum(abs(nationalPredictions_JMP_Oceania$JMP.reported - nationalPredictions_JMP_Oceania$nationalSMDWcoverage))/length(nationalPredictions_JMP_Oceania$JMP.reported)
MAE_singlePred_Oceania

##for Oceania SMDW from main model (ensemble prediction) and JMP estimates

r2_JMP_ensemblePred_Oceania <- 1 - (sum((nationalPredictions_JMP_Oceania$JMP.reported-nationalPredictions_JMP_Oceania$nationalSMDWcoverage_mean)^2)/sum((nationalPredictions_JMP_Oceania$JMP.reported-mean(nationalPredictions_JMP_Oceania$JMP.reported))^2))
r2_JMP_ensemblePred_Oceania

MAE_ensemblePred_Oceania <- sum(abs(nationalPredictions_JMP_Oceania$JMP.reported - nationalPredictions_JMP_Oceania$nationalSMDWcoverage_mean))/length(nationalPredictions_JMP_Oceania$JMP.reported)
MAE_ensemblePred_Oceania


```

# 10. Comparison of final random forest model predictions to averaged prediction of 100 spatially stratified RF models
