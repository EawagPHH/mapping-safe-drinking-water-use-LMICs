...
"C:/Users/Esther/Documents/GitHub/MICS_SMDW/Feature Selection")
...
We pre-elected 117 variables based on their relevance for modelling safely managed drinking water as anthropogenic, hydrogeologic, climatic, topographic and biogeographic indicators from a list of 384 variables used in ecological modelling (compiled by the Crowther Lab). 
...
random_points_data <- fread("/Users/Esther/Documents/GitHub/MICS_SMDW/Feature Selection/subs100k.csv") %>% mutate_all(as.numeric)

#select variables 
random_points <- random_points_data %>% select(all_of(bandNames)) %>%
  na.omit() #loose about 20000 observations which have NAs
```
#PCA for determining the number of dimensions needed to describe the variation in the dataset

```{r}
res.pca <- prcomp((random_points), scale = TRUE)

fviz_screeplot(res.pca, addlabels = TRUE, ylim = c(0, 50))

get_eig(res.pca) %>% 
  rownames_to_column(var = "Dimensions") %>%
  mutate(Dimensions = as.numeric(str_remove(Dimensions, "Dim."))) %>% 
  arrange(Dimensions) %>% 
  ggplot(aes(x = Dimensions, y = cumulative.variance.percent)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90))

#Results taken from table:
# Dimensions to describe 80% of variation: 24 
# Dimensions to describe 90% of variation: 39
# Dimensions to describe 95% of variation: 51
# Dimensions to describe 99% of variation: 73
# Dimensions to describe 99.9% of variation: 91
...
for (k in c(24,39,51,73,91)){
  ...
  df %>% fwrite(paste0('/Users/Esther/Documents/GitHub/MICS_SMDW/Feature Selection/clustofvar_50k_',k,'.csv'))
